					...
					<div class="col-sm-3">
						...
						<select name="module" id="module" class="form-control nbr">
							...
							<?php $__currentLoopData = $modules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $module): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
								<option value="<?php echo e($module->id); ?>"><?php echo e($module->m_name); ?></option>
		...
		<table class="datatable table table-striped table-hover" id="dataTable">
			...
			<tbody>
				...
				<tr>
					...
					<td class="text-center"><span class="badge badge-danger"><?php echo e($role->users->count()); ?></span></td>
					<td class="text-right"><button type="button" onclick="permission('<?php echo e($role->id); ?>', 'btn-permission')" data-toggle="modal" data-target=".bs-example-modal-lg" title="Permission" id="permission" class="btn btn-info btn-sm"><i class="fa fa-user-graduate"></i> &nbsp;សិទ្ធិ</button></td>
				</tr>
				<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
			</tbody>
		</table>
...
<?php $__env->startSection('js'); ?>
	<script type="text/javascript">
		...
		function permission(ur_id,type) {
			...
			// Request using Ajax
			$.ajax({
				url: "<?php echo e(route('permissions.set_permission')); ?>",
				type: 'post',
		...
		function update_permission() {
			$('input[type="checkbox"]').change(function () {
				...
				// Request using Ajax
				$.ajax({
					url: "<?php echo e(route('permissions.update_permission')); ?>",
					type: 'post',
