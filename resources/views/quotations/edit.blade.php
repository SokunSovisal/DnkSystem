...
@section('content')
	...
	{!! Form::open(['url' => route('quotations.update', $quote->id)]) !!}
					...
					<div class="col-sm-6">
						...
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('quote_cp_name'))?'has-error':'')}}">
								...
								<input class="form-control nbr" type="text" name="quote_cp_name" id="quote_cp_name" placeholder="contact person" value="{{ ((count($errors) > 0) ? old('quote_cp_name') : $quote->quote_cp_name) }}" autocomplete="off" required />
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('quote_cp_phone'))?'has-error':'')}}">
								<label class="control-label">លេខទូរស័ព្ទ <small>*</small></label>
								<input class="form-control nbr" type="text" name="quote_cp_phone" id="quote_cp_phone" placeholder="attend's phone" value="{{ ((count($errors) > 0) ? old('quote_cp_phone') : $quote->quote_cp_phone) }}" autocomplete="off" required />
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('quote_cp_email'))?'has-error':'')}}">
								<label class="control-label">អ៊ីមែល</label>
								<input class="form-control nbr" type="email" name="quote_cp_email" id="quote_cp_email" placeholder="attend's email" value="{{ ((count($errors) > 0) ? old('quote_cp_email') : $quote->quote_cp_email) }}" autocomplete="off"/>
							</div>
						</div>
					</div><!-- /.column -->
					...
					<div class="col-sm-6">
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('quote_number'))?'has-error':'')}}">
								...
								<input class="form-control nbr" type="text" name="quote_number" placeholder="quotation number" value="{{ ((count($errors) > 0) ? old('quote_date') : $quote->quote_number) }}" autocomplete="off" disabled />
							</div>
						</div>
						...
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('quote_date'))?'has-error':'')}}">
								...
								<div class='input-group date'>
									<input class="form-control nbr" type="text" id='datepicker' name="quote_date" placeholder="date & time" value="{{ ((count($errors) > 0) ? old('quote_date') : $quote->quote_date) }}" autocomplete="off" required data-mask="9999-99-99" />
						...
						<div class="col-sm-12">
							<div class="form-group">
								...
								<select name="quote_status" class="form-control nbr" required>
									...
									<option value="1" {{ ($quote->quote_status ==1 || old('quote_status')==1) ? 'selected':'' }}>មិនទាន់មានចម្លើយ</option>
									<option value="2" {{ ($quote->quote_status ==2 || old('quote_status')==2) ? 'selected':'' }}>ជោគជ័យ</option>
									<option value="3" {{ ($quote->quote_status ==3 || old('quote_status')==3) ? 'selected':'' }}>មិនជោគជ័យ</option>
								</select>
							</div>
						</div>
					</div><!-- /.column -->
					...
					<div class="col-sm-12">
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('quote_purpose'))?'has-error':'')}}">
								...
								<input class="form-control nbr" type="text" name="quote_purpose" placeholder="purpose" value="{{ ((count($errors) > 0) ? old('quote_purpose') : $quote->quote_purpose) }}" autocomplete="off" required />
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group">
								<label for="">រយៈពេលនិងលក្ខខណ្ឌ</label>
								<textarea class="form-control" name="quote_term" id="myEditor">{{ $quote->quote_term}}</textarea>
							</div>
						</div>
					</div><!-- /.column -->
				</div><!--  /.row -->
