					...
					<tbody>
						@foreach($files as $i => $file)
							<tr>
								<td>{{ $i+1 }}</td>
								<td>
									<a class="btn btn-sm btn-success waves-effect waves-light" href="{{(substr(strrchr($file->f_name,'.'),1) == 'pdf') ? route('files.pdf',$file->id) : '/documents/'.$file->f_company_id.'/'.$file->f_name}}">{!!(substr(strrchr($file->f_name,'.'),1) == 'pdf') ? '<i class="fa fa-file-pdf"></i>' : '<i class="fa fa-image"></i>'!!} {{ substr($file->f_name, strpos($file->f_name, "_") + 1) }}</a>
								</td>
