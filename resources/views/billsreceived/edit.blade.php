...
@section('content')
	...
	{!! Form::open(['url' => route('billsreceived.update', $billsreceived->id)]) !!}
				...
				<div class="row">
					...
					<div class="col-sm-12">
							<div class="form-group {{(($errors->has('br_number'))?'has-error':'')}}">
								...
								<input class="form-control" type="text" name="br_number" placeholder="bill number" value="{{ ((count($errors) > 0) ? old('br_number') : $billsreceived->br_number) }}" autocomplete="off" required />
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group">
								...
								<select name="br_company_id" id="br_company_id" class="form-control select2" required>
									...
									@foreach($companies as $i => $com)
										<option value="{{$com->id}}" {{ ($com->id == old('br_company_id') || $com->id == $billsreceived->br_company_id) ? 'selected':'' }}>{{$com->com_name}}</option>
									@endforeach
								</select>
							</div>
						</div>
					...
					<div class="col-sm-6">
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('br_date'))?'has-error':'')}}">
								...
								<div class='input-group date'>
									<input class="form-control" type="text" id='datepicker' name="br_date" placeholder="date & time" value="{{ ((count($errors) > 0) ? old('br_date') : $billsreceived->br_date) }}" autocomplete="off" required data-mask="9999-99-99" />
						...
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('br_total'))?'has-error':'')}}">
								...
								<input class="form-control" type="number" step="0.01" min="0" name="br_total" placeholder="bill amount" value="{{ ((count($errors) > 0) ? old('br_total') : $billsreceived->br_total) }}" autocomplete="off" required />
							</div>
						</div>
					</div><!-- /.column -->
					...
					<div class="col-sm-12">
						<div class="col-sm-12">
							<div class="form-group">
								...
								<textarea class="form-control" name="br_description" id="myEditor">{{ ((count($errors) > 0) ? old('inv_description') : $billsreceived->br_description) }}</textarea>
							</div>
						</div>
					</div><!-- /.column -->
				</div><!--  /.row -->
