				...
				<tbody>
					@foreach($processes as $i => $process)
						<tr>
							...
							<td>{{ $process->service->s_name }}</td>
							<td>{{ $process->pr_description }}</td>
							<td class="action text-right">
								<a href="{{route('process.edit',$process->id)}}" title="Edit" class="edit btn btn-info btn-sm"><i class="fa fa-pencil-alt"></i></a>
								{{Form::open(['url'=>route('process.destroy',$process->id)])}}
