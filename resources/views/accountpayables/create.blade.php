...
@section('content')
	{!! Form::open(['url' => route('accountpayables.store')]) !!}
					...
					<div class="col-sm-6">
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('pt_date'))?'has-error':'')}}">
								...
								<div class='input-group date'>
									<input class="form-control" type="text" id='datepicker' name="pt_date" placeholder="date & time" value="{{ ((count($errors) > 0) ? old('pt_date') : '') }}" autocomplete="off" required data-mask="9999-99-99" /> <span class="nbr input-group-addon">
						...
						<div class="col-sm-12">
							<div class="form-group">
								<label class="control-label">បង់ទៅកាន់វិក្កយបត្រ <small>*</small></label>
								<select name="pt_bill_id" id="pt_bill_id" class="form-control select2" required>
									<option value="">-- ជ្រើសរើសវិក្កយបត្រ --</option>
									@foreach($bills as $i => $bill)
										<option value="{{$bill->id}}" {{ ($bill->id == old('pt_bill_id')) ? 'selected':'' }}>{{$bill->br_number}}</option>
									@endforeach
								</select>
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group">
								...
								<textarea class="form-control" name="pt_description" style="height: 108px;" placeholder="description">{{ ((count($errors) > 0) ? old('inv_description') : '') }}</textarea>
							</div>
						</div>
					</div><!-- /.column -->
					...
					<div class="col-sm-6">
							...
							<div class="form-group">
								...
								<select name="company_id" id="company_id" class="form-control select2" disabled>
									...
									@foreach($companies as $i => $com)
										<option value="{{$com->id}}" {{ ($com->id == old('company_id')) ? 'selected':'' }}>{{$com->com_name}}</option>
									@endforeach
								</select>
							</div>
						</div>
						...
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('pt_amount'))?'has-error':'')}}">
								...
								<input class="form-control" type="number" step="0.01" id="pt_amount" min="0" name="pt_amount" placeholder="paid amount" value="{{ ((count($errors) > 0) ? old('pt_amount') : '') }}" autocomplete="off" required />
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group">
