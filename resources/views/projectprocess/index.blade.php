			...
			<table class="table table-striped table-hover" id="dataTable">
				...
				<tbody>
					@foreach($transactions as $tr=> $transaction)
						<tr>
							<td>{{ ++$tr }}</td>
							<td>{{ $transaction->tr_start_date }}</td>
							<td>{{ $transaction->company->com_name }}</td>
							<td>{{ $transaction->service->s_name }}</td>
							<td>{{ $transaction->verify_by->name }}</td>
							<td class="action text-right">
								<button class="btn btn-sm btn-warning" onclick="getTrID( '{{ $transaction->id }}' )" data-toggle="modal" data-target=".bs-process-model"><i class="fa fa-shoe-prints"></i></button>
								<a href="{{route('projectprocess.edit', $transaction->id)}}" title="Edit" class="edit btn btn-sm btn-info"><i class="fa fa-pencil-alt"></i></a>
								{{Form::open(['url'=>route('projectprocess.destroy', $transaction->id)])}}
...
@section('js')
	<script type="text/javascript">
		...
		function getTpID(pr_id, tr_id, pr_name) {
			...
			$.ajax({
				url: "{{route('projectprocess.ajaxfindtp')}}",
				type: 'post',
				...
				success: function(result){
					...
					$('#submit_new_tp').click(function () {
					  ...
					  if ($('#tp_status').is(':checked')) {
						...
						var _token = $('input[name="_token"]').val();
						$.ajax({
							url: "{{route('projectprocess.ajaxstoretp')}}",
							type: 'post',
					...
					$('#submit_update_tp').click(function () {
					  ...
					  if ($('#tp_status').is(':checked')) {
						...
						var _token = $('input[name="_token"]').val();
						$.ajax({
							url: "{{route('projectprocess.ajaxupdatetp')}}",
							type: 'post',
		...
		function getTrID(id) {
			var _token = $('input[name="_token"]').val();
			$.ajax({
				url: "{{route('projectprocess.ajaxtp')}}",
				type: 'post',
