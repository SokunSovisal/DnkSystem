...
@section('content')
	{!! Form::open(['url' => route('projectprocess.store')]) !!}
		<section class="bg-white">
			...
			<br/>
				<div class="row">
					<div class="col-sm-6">
						<div class="col-sm-12">
							<div class="form-group {{ (($errors->has('tr_start_date'))?'has-error':'' ) }}">
								<label class="control-label">ការបរិច្ឆេទ <small>*</small></label>
								<div class='input-group date'>
									<input class="form-control" type="text" id='tr_start_date' name="tr_start_date" placeholder="date & time" value="{{ ((count($errors) > 0) ? old('tr_start_date') : '') }}" autocomplete="off" required data-mask="9999-99-99" />
						...
						<div class="col-sm-12">
							<div class="form-group {{ (($errors->has('tr_date_count'))?'has-error':'' ) }}">
								...
								<div class='input-group date'>
									<input class="form-control" type="number" pattern="[0-9]" min="0" id='tr_date_count' name="tr_date_count" placeholder="estimate time" value="{{ ((count($errors) > 0) ? old('tr_date_count') : '') }}" autocomplete="off" />
								  <span class="input-group-addon">
									  ថ្ងៃ
								  </span>
								</div>
                ...
                <select name="tr_invoice_id" class="form-control select2" id="tr_invoice_id">
                  ...
                  @foreach($invoices as $i => $invoice)
                    <option value="{{$invoice->id}}"  {{ (($invoice->id == old('tr_invoice_id'))? 'selected':'') }}>{{$invoice->inv_number}}</option>
                  @endforeach
                </select>
							</div>
						</div>
                ...
                <select name="tr_company_id" class="form-control select2" id="tr_company_id" required>
                  ...
                  @foreach($companies as $i => $company)
                    <option value="{{$company->id}}"  {{ (($company->id == old('tr_company_id'))? 'selected':'') }}>{{$company->com_name}}</option>
                  @endforeach
                </select>
							</div>
						</div>
                ...
                <select name="tr_service_id" class="form-control select2" id="tr_service_id" required>
                  ...
                  @foreach($services as $i => $service)
                    <option value="{{$service->id}}"  {{ (($service->id == old('tr_service_id'))? 'selected':'') }}>{{$service->s_name}}</option>
                  @endforeach
                </select>
							</div>
						</div>
					...
					<div class="col-sm-6">
						<div class="col-sm-12">
							<div class="form-group {{ (($errors->has('tr_date_alert'))?'has-error':'' ) }}">
								<label class="control-label">ការបរិច្ឆេទជូនដំណឹង <small>*</small></label>
								<div class='input-group date'>
									<input class="form-control" type="text" id='tr_date_alert' name="tr_date_alert" placeholder="date & time" value="{{ ((count($errors) > 0) ? old('tr_date_alert') : '') }}" autocomplete="off" required data-mask="9999-99-99" />
								  <span class="input-group-addon">
									  <span class="glyphicon glyphicon-calendar"></span>
								  </span>
								</div>
                  ...
                  @endforeach
						...
						<div class="col-sm-12">
							<div class="form-group">
								...
								<textarea class="form-control" name="tr_description"  style="height: 183px;" placeholder="description">{{ ((count($errors) > 0) ? old('tr_description') : '') }}</textarea>
							</div>
						</div>
					</div><!-- /.column -->
				</div><!--  /.row -->
