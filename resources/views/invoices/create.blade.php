...
@section('content')
	{!! Form::open(['url' => route('invoices.store')]) !!}
					...
					<div class="col-sm-6">
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('inv_number'))?'has-error':'')}}">
								...
								<input class="form-control nbr" type="text" name="inv_number" placeholder="invoice number" value="{{ ((count($errors) > 0) ? old('inv_number') : (isset($invoice->inv_number)? str_pad($invoice->inv_number+1, 6, '0', STR_PAD_LEFT): '000001')) }}" autocomplete="off" required />
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group">
								...
								<select name="inv_company_id" id="inv_company_id" class="form-control nbr select2" required>
									...
									@foreach($companies as $i => $com)
										<option value="{{$com->id}}" {{ ($com->id == old('inv_company_id')) ? 'selected':'' }}>{{$com->com_name}}</option>
									@endforeach
								</select>
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('inv_com_phone'))?'has-error':'')}}">
								...
								<input class="form-control nbr" type="text" name="inv_com_phone" id="inv_com_phone" placeholder="attend's phone" value="{{ ((count($errors) > 0) ? old('inv_com_phone') : '') }}" autocomplete="off" />
							</div>
						</div>
					</div><!-- /.column -->
						...
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('inv_date'))?'has-error':'')}}">
								...
								<div class='input-group date'>
									<input class="form-control nbr" type="text" id='datepicker' name="inv_date" placeholder="date & time" value="{{ ((count($errors) > 0) ? old('inv_date') : '') }}" autocomplete="off" required data-mask="9999-99-99" />
							...
							<div class="form-group {{(($errors->has('inv_vat_status'))?'has-error':'')}}">
								...
								<select name="inv_vat_status" id="inv_vat_status" class="form-control nbr" required>
									<option value="">-- ជ្រើសរើសប្រភេទវិក្កយបត្រ --</option>
										<option value="1" {{ (old('inv_vat_status')=="1") ? 'selected':'' }}>វិក្កយបត្រធម្មតា</option>
										<option value="2" {{ (old('inv_vat_status')=="2") ? 'selected':'' }}>វិក្កយបត្រអាករ</option>
								</select>
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('inv_quote_refer'))?'has-error':'')}}">
								...
								<select name="inv_quote_refer" id="inv_quote_refer" class="form-control nbr select2">
									...
									@foreach($quotations as $i => $quote)
										<option value="{{$quote->id}}" {{ ($quote->id == old('inv_quote_refer')) ? 'selected':'' }}>{{$quote->quote_number}}</option>
									@endforeach
								</select>
							</div>
						</div>
					...
					<div class="col-sm-12">
						<div class="col-sm-12">
							<div class="form-group {{(($errors->has('inv_com_address'))?'has-error':'')}}">
								...
								<input class="form-control nbr" type="text" name="inv_com_address" id="inv_com_address" placeholder="company address" value="{{ ((count($errors) > 0) ? old('inv_com_address') : '') }}" autocomplete="off"/>
							</div>
						</div>
						<div class="col-sm-12">
							<div class="form-group">
								<label for="">រយៈពេលនិងលក្ខខណ្ឌ</label>
								<textarea class="form-control" name="inv_description" id="myEditor">{{ ((count($errors) > 0) ? old('inv_description') : '') }}</textarea>
							</div>
						</div>
					</div><!-- /.column -->
				</div><!--  /.row -->
